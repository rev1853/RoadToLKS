...
namespace first.Controllers
{
    ...
    public class WeatherForecastController(EsemkaHeroContext context) : ControllerBase
    {
        ...
        [HttpGet("/")]
        public List<Clan> Get([FromQuery] string?name)
        {
            if (!string.IsNullOrEmpty(name)) return this.context.Clans.Where(f => f.Name.Contains(name)).ToList();
            else return [.. this.context.Clans];
        }
